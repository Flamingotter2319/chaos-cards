// ══════════════════════════════════════════════════════════
//  CHAOS CARDS — Server v4 (MongoDB Atlas)
//
//  Required env variable:
//    MONGODB_URI=mongodb+srv://user:pass@cluster.mongodb.net/chaoscards
//
//  Set this in Render → your service → Environment
// ══════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════
let db = null;
let accounts = null; // MongoDB collection

async function connectDB() {
  if (!MONGO_URI) {
    console.warn('⚠️  No MONGODB_URI set — accounts will be stored in memory only (lost on restart)');
    return;
  }
  try {
    const client = new MongoClient(MONGO_URI);
    await client.connect();
    db = client.db('chaoscards');
    ...
    console.warn('Falling back to in-memory accounts');
  }
}
...
async function getAccount(username) {
  if (accounts) return await accounts.findOne({ username });
  return memAccounts[username] || null;
}

async function createAccount(username, passwordHash) {
  const doc = {
    username, passwordHash,
    chips: START_CHIPS, lastBroke: null,
    totalWon: 0, gamesPlayed: 0,
    createdAt: new Date(),
  };
  if (accounts) {
    await accounts.insertOne(doc);
  } else {
    memAccounts[username] = doc;
  }
  return doc;
}
...
async function updateAccount(username, updates) {
  if (accounts) {
    await accounts.updateOne({ username }, { $set: updates });
  } else {
    if (memAccounts[username]) Object.assign(memAccounts[username], updates);
  }
}
...
const loggedIn = new Map();  // username → playerId
let uidCounter = 1;
const nextId = () => String(uidCounter++);
...
function buildDeck(deckCount, enabledSpecials) {
  ...
  const specials = Array.isArray(enabledSpecials) && enabledSpecials.length > 0
...
function startGame(room) {
  ...
  const game = {
    ...
    phase: 'betting', currentPlayerIdx: 0, skipNext: false,
...
function checkAllBet(room) {
  ...
  if (game.settings.mode === 'blackjack') {
    ...
    game.players.forEach((_,i) => { dealTo(game,i); dealTo(game,i); });
    for (let i=0;i<5;i++) dealComm(game); // flop(3)+turn+river
  }
...
function endRound(room) {
  ...
  game.players.forEach(gp => {
    ...
    if (rp&&rp.username) {
      updateAccount(rp.username, {
        chips: gp.chips,
        gamesPlayed: null, // handled below
      }).catch(console.error);
      // Increment counters separately
      if (accounts) {
        accounts.updateOne({ username:rp.username }, {
          $set:{ chips:gp.chips },
          $inc:{ gamesPlayed:1, ...(gp.status==='winner'?{ totalWon:1 }:{}) }
        }).catch(console.error);
      }
    }
  });
  ...
  setTimeout(() => {
    ...
    room.players.forEach(p => {
      ...
      if (!p.isBot&&p.chips<=0) {
        p.spectating=true;
        const now=Date.now();
        if (p.username) updateAccount(p.username, { chips:0, lastBroke:now }).catch(console.error);
...
wss.on('connection', ws => {
  ...
  ws.on('message', async raw => {
    ...
    if (msg.type==='register') {
      ...
      if (!u||!p||u.length<3||p.length<4) {
        sendTo(ws,{ type:'authError',message:u.length<3?'Username needs 3+ characters.':p.length<4?'Password needs 4+ characters.':'Fill in all fields.' }); return;
      }
    ...
    if (msg.type==='login') {
      ...
      if (acc.chips<=0&&acc.lastBroke) {
        ...
        await updateAccount(u,{ chips:BROKE_GIFT,lastBroke:null });
        acc.chips=BROKE_GIFT;
      }
    ...
    switch(msg.type) {
      ...
      case 'addBot': {
        ...
        const botId=nextId(),types=['simple','smart','chaotic'],btype=types[Math.floor(Math.random()*3)];
        ...
        broadcast(room.id,{ type:'playerJoined',player:{ id:botId,name:bname,chips:500,isBot:true,botType:btype },server:roomSnapshot(room) });
        broadcastSrvList(); break;
      }
      case 'removeBot': {
  ...
  ws.on('close',()=>{
    ...
    if (m) { handleLeave(ws,m); if(m.username){ if(loggedIn.get(m.username)===m.playerId) loggedIn.delete(m.username); } }
    clients.delete(ws);
  });
});
...
function handleLeave(ws,meta){
  ...
  if (room.players.size===0){ servers.delete(room.id); }
    ...
    if (room.game){ const gp=room.game.players.find(p=>p.id===meta.playerId);
